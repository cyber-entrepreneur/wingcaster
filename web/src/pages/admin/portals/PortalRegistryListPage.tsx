      ...
      },
      {
        ...
        cell: (row) => (
          <div className="flex flex-col">
            ...
            >
              {relativeTime(row.last_change_at, locale === 'ar' ? 'ar' : 'en')}
            </span>
            {row.last_change_by ? (
  ...
  return (
    ...
    >
      ...
      <PAQueueFilterStrip
        ...
        onChange={(next) => {
          if (next.status !== status) patchParams({ status: next.status === 'all' ? null : next.status })
          if (next.search !== searchDraft) setSearchDraft(next.search)
        }}
        customFilters={<>
      ...
      <PAQueueKeyboardShortcutsPanel
        ...
        shortcuts={[
          { keys: 'J', description: t('kbd.next') },
          { keys: 'K', description: t('kbd.prev') },
          { keys: 'Enter', description: t('kbd.open') },
          { keys: 'E', description: t('kbd.edit') },
          { keys: 'H', description: t('kbd.history') },
          { keys: 'A', description: t('kbd.add') },
          { keys: '.', description: t('kbd.refresh') },
          { keys: '?', description: t('kbd.help') },
          { keys: 'Esc', description: t('kbd.escape') },
        ]}
      />
    </div>
  )
