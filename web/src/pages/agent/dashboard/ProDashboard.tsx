...
export interface ProDashboardProps {
  ...
  greetingName?: string
  /** Alias used by AgentDashboardModeMount. */
  agentName?: string
  className?: string
}
 ...
 */
export function ProDashboard({ stats: statsProp, greetingName, agentName, className }: ProDashboardProps) {
  const { agent } = useAuth()
  ...
  const name =
    ...
    agentName ||
    agent?.name?.split(' ')[0] ||
    t('greeting.nameFallback', copyLocale)
  const greetingEn = useMemo(() => greetingForHour(new Date().getHours()), [])
  const headingText =
    copyLocale === 'ar'
      ? t('greeting.goodDay', 'ar', { name })
      : `${greetingEn}, ${name}`
