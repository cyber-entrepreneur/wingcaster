...
const localeMock = vi.hoisted(() => ({
  ...
  dir: 'ltr' as 'ltr' | 'rtl',
  isArabic: false,
  setLocale: vi.fn(async (_next: 'en' | 'ar') => ({ ok: true as const })),
}))
...
const ACTIVATION_CODE = {
  ...
  expires_at: '2099-09-09T12:15:00.000Z',
}
...
type ApiStore = {
  ...
  agencyInvites: Array<{ id: string; email: string; sent_at: string; status: string }>
  agencyShare: { url: string; code: string } | null
}
...
function installFetch() {
  fetchMock.mockImplementation(async (url: unknown, init?: RequestInit) => {
    ...
    if (pathUrl.includes('/users/me') && method === 'PATCH') {
      return jsonRes(200, { ...(typeof body === 'object' && body ? body : {}), ok: true })
    }
    if (pathUrl.includes('/agency/invitations/share-link')) {
      return jsonRes(
        200,
        store.agencyShare ?? {
          url: 'https://wingcaster.app/join/acme?code=INV-1',
          code: 'INV-1',
        },
      )
    }
    if (pathUrl.includes('/agency/invitations') && method === 'GET') {
      return jsonRes(200, store.agencyInvites)
    }
    if (pathUrl.includes('/users/me/onboarding-events') || pathUrl.includes('/agent/onboarding-events')) {
      return jsonRes(202, { accepted: true })
    }
...
/** In-router navigation helper for mid-steps that cannot auto-advance in one session. */
function TestNavigate({ to, label = 'advance' }: { to: string; label?: string }) {
  const navigate = useNavigate()
  return (
    <button type="button" data-testid={`test-nav-${label}`} onClick={() => navigate(to)}>
      {label}
    </button>
  )
}

function FunnelTestNavBar() {
  return (
    <div data-testid="funnel-test-nav" hidden>
      <TestNavigate to="/onboarding/whatsapp/waiting" label="to-waiting" />
      <TestNavigate to="/onboarding/whatsapp/drafting/sess_1" label="to-drafting" />
      <TestNavigate to="/onboarding/first-listing/draft_1" label="to-review" />
      <TestNavigate to="/onboarding/first-listing/published" label="to-celebration" />
    </div>
  )
}

function FunnelRoutes({ fallback }: { fallback: ReactElement }) {
  return (
    <Routes>
      <Route path="/register" element={<RegisterPage />} />
      <Route path="/onboarding" element={pages.welcome ? <pages.welcome /> : <Placeholder label="onboarding" />} />
      <Route path="/onboarding/welcome" element={pages.welcome ? <pages.welcome /> : fallback} />
      <Route
      ...
      />
      <Route path="/activate" element={pages.actWelcome ? <pages.actWelcome /> : fallback} />
      <Route path="/activate/welcome" element={pages.actWelcome ? <pages.actWelcome /> : fallback} />
      <Route
      ...
      />
      <Route path="/channels" element={<Placeholder label="channels" />} />
...
/** Remounting helper — cleans up previous RTL tree first. */
function wrap(ui: ReactElement, initialPath: string, extra?: ReactElement) {
  cleanup()
  return mountFunnel(initialPath, ui, extra)
}

/** Single-session mount — does NOT cleanup; keep the same MemoryRouter across navigations. */
function mountFunnel(initialPath: string, fallbackUi?: ReactElement, extra?: ReactElement) {
  const ui = fallbackUi ?? <Placeholder label="fallback" />
  return render(
    <MemoryRouter initialEntries={[initialPath]}>
      <BrandProvider>
        <ToastProvider>
          <LocationProbe />
          <FunnelStateProbe />
          <FunnelRoutes fallback={ui} />
          {extra ?? <FunnelTestNavBar />}
        </ToastProvider>
      </BrandProvider>
    </MemoryRouter>,
  )
...
beforeEach(() => {
  ...
  localeMock.setLocale.mockReset()
  localeMock.setLocale.mockImplementation(async (next: 'en' | 'ar') => {
    localeMock.locale = next
    localeMock.isArabic = next === 'ar'
    localeMock.dir = next === 'ar' ? 'rtl' : 'ltr'
  ...
  registerApiMock.postAuthRegister.mockReset()
  registerApiMock.adoptRegisterToken.mockReset()
  registerApiMock.postAuthRegister.mockResolvedValue({
    user: {
      id: 'usr_sara',
      display_name: 'Sara',
      identifier_type: 'email',
      identifier_masked: 's***@wingcaster.test',
    },
    tenant: { id: 'personal:usr_sara', name: 'personal', role: 'personal' },
    session: { token: 'tok_sara', expires_at: '2099-01-01' },
    redirect_to: '/onboarding/welcome',
    otp_transport: 'microsoft_graph',
  })
  registerApiMock.adoptRegisterToken.mockImplementation(async () => {
    authMock.agent = { ...DEFAULT_AGENT, preferred_locale: localeMock.locale }
    await authMock.refreshAgent()
  })
})
  ...
  it('illegal PATCH 409 does not crash; user is treated as already onboarded', async () => {
    ...
    await act(async () => {
      try {
        ...
        returned = (err as PatchConflictError).state
      }
    })
...
describe.skipIf(!familyReady('onb'))(
  '0. Solo register (ar locale, microsoft_graph OTP) → /onboarding/welcome',
  () => {
    it('registers with locale ar, Graph OTP transport, then lands on Welcome', async () => {
      const user = userEvent.setup()
      authMock.agent = null
      ...
      const body = registerApiMock.postAuthRegister.mock.calls[0][0] as { locale?: string; path?: string }
      expect(body.path).toBe('solo')
      expect(body.locale).toBe('ar')

      const registerResult = await registerApiMock.postAuthRegister.mock.results[0]?.value
      expect(registerResult?.otp_transport).toBe('microsoft_graph')
      expect(registerResult?.otp_transport).not.toBe('resend')

      expect(registerApiMock.adoptRegisterToken).toHaveBeenCalledWith('tok_sara')
      expect(authMock.refreshAgent).toHaveBeenCalled()

      await localeMock.setLocale('ar')
      const localePatch = fetchMock.mock.calls.find((call) => {
        const url = requestUrl(call[0])
        const init = call[1] as RequestInit | undefined
        return url.includes('/users/me') && String(init?.method || '').toUpperCase() === 'PATCH'
      })
      expect(localePatch).toBeTruthy()
      expect(String((localePatch?.[1] as RequestInit | undefined)?.body || '')).toMatch(
        /preferred_locale":"ar"/,
      )

      await waitFor(() => {
        expect(screen.getByTestId('funnel-location').textContent).toContain('/onboarding/welcome')
      }, { timeout: 5000 })
      expect(
        await screen.findByRole('heading', {
          name: /Welcome to WingCaster|مرحبًا بك في وينغكاستر/i,
        }),
      ).toBeInTheDocument()
    })
  },
...
describe.skipIf(!familyReady('wlb'))(
  '2. WhatsApp path WLB-001→005 → ONB-003 review → publish → ONB-004 (single session)',
  () => {
    it('walks connect → code → waiting → drafting → review → celebration without remounts', async () => {
      const user = userEvent.setup()
      store.onboarding = welcomeOnboarding({
      ...
      store.inboundReady = false
      store.drafts = [SAMPLE_DRAFT]

      cleanup()
      mountFunnel('/onboarding/whatsapp/connect')
      await waitFor(() => expect(screen.queryByTestId('funnel-loading')).not.toBeInTheDocument())
      ...
      // Same MemoryRouter: mutate store then in-router navigate (avoid binding-poll remount loops).
      store.binding = 'bound'
      await user.click(screen.getByTestId('test-nav-to-waiting'))
      await waitFor(() => {
        expect(screen.getByTestId('funnel-location').textContent).toMatch(/\/onboarding\/whatsapp\/waiting/)
      }, { timeout: 5000 })
      expect(
        await screen.findByRole('heading', { name: /Listening on WhatsApp/i }),
      ).toBeInTheDocument()

      store.inboundReady = true
      await user.click(screen.getByTestId('test-nav-to-drafting'))
      await waitFor(() => {
        expect(screen.getByTestId('funnel-location').textContent).toMatch(
          /\/onboarding\/whatsapp\/drafting/,
        )
      }, { timeout: 5000 })
      expect(
        (await screen.findAllByText(/Turning your message into a listing|Your listing is ready|Review & publish|Drafting your listing/i))
          .length,
      ).toBeGreaterThan(0)
      ...
      await user.click(screen.getByTestId('test-nav-to-review'))
      await waitFor(() => {
        expect(screen.getByTestId('funnel-location').textContent).toMatch(
          /\/onboarding\/first-listing\/draft_1/,
        )
      }, { timeout: 5000 })
...
describe.skipIf(!familyReady('onb'))(
  '4. Mid-funnel resume (whatsapp_intake_pending / ACT-002) via resumeRouteForStep',
  () => {
    it('Welcome remount redirects to /onboarding/whatsapp and FunnelStateProbe shows mid step', async () => {
      store.onboarding = welcomeOnboarding({
        step: 'whatsapp_intake_pending',
        path: 'whatsapp',
        checklist: { ...EMPTY_CHECKLIST, welcome_seen: true },
      ...
      await waitFor(() => {
        expect(screen.getByTestId('funnel-state').getAttribute('data-step')).toBe(
          'whatsapp_intake_pending',
        )
        expect(screen.getByTestId('funnel-location').textContent).toMatch(/\/onboarding\/whatsapp/)
      }, { timeout: 5000 })
    })
  },
)
...
describe.skipIf(!familyReady('act'))(
  '8. Agency-owner persona — invite_team → channels (+ celebrate when final)',
  () => {
    it('completes invite_team and navigates to /channels with celebrate=1', async () => {
      const user = userEvent.setup()
      authMock.agent = { ...DEFAULT_AGENT, role: 'owner', agency_name: 'Elite RE' }
      store.activation = activationDoc({
        signup_path: 'agency',
        stepOverrides: {
          whatsapp: {
      ...
      store.agencyInvites = [
        { id: 'inv_1', email: 'pa@example.com', sent_at: '2026-09-08T10:00:00Z', status: 'pending' },
      ]
      store.agencyShare = {
        url: 'https://wingcaster.app/join/elite?code=INV-1',
        code: 'INV-1',
      }

      const Invite = pages.actInvite
      if (!Invite) return
      wrap(<Invite />, '/activate/invite-team')
      await waitFor(() => expect(screen.queryByTestId('funnel-loading')).not.toBeInTheDocument())
...
describe('Wave 4A Phase A discovery (hard fail if families missing)', () => {
  it('requires onb, wlb, act families and dashboard checklist mount', () => {
    expect(familyReady('onb')).toBe(true)
    expect(familyReady('wlb')).toBe(true)
    expect(familyReady('act')).toBe(true)
    expect(dashboardMountsChecklist()).toBe(true)
  })
})
