...
const INLINE_HEX = /#[0-9A-Fa-f]{3,8}\b/

function inlineHexHits(root: HTMLElement): string[] {
  return [...root.querySelectorAll('[style]')]
    .map((el) => el.getAttribute('style') || '')
    .filter((style) => INLINE_HEX.test(style))
}
...
describe.each(['light', 'dark'] as const)('Broadcast %s mode — existing screens', (mode) => {
  ...
  it(`uses Broadcast page background token and has no inline hex (${mode})`, () => {
    applyLcMode(mode)
    const view = mount(LoginPage)
    const page = getComputedStyle(document.documentElement).getPropertyValue('--lc-bg-page').trim()
