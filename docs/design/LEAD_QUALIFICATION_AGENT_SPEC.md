...
## Scope + timeline decision (WingCaster planning note)
Go-live is **2026-09-30** (`project_go_live_sept_30` memory). **DECIDED 2026-09-18: this agent is a FAST-FOLLOW — NOT in the Sept-30 launch.** The launch is the base platform (existing waves); this program is delivered phase-by-phase afterward and ships when its eval gates pass on real conversation data (weeks post-launch, since live traffic can't begin before Phase 5 and Phase 9 lags by design). A thin Phase-0/1 slice (schema + loop + in-app adapter + versioning + a hand-written `[20]`-conversation golden set) may start in parallel but is **not** gated to launch. Per-open-item ratification of the 14 items precedes Phase 1.
