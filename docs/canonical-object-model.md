...
### H. Measurement
- **Event** 🆕 **[L]** — the append-only spine (discrete facts only). Hardened per taxonomy v2:
  `id (=event identity) · event_name · event_category(business/delivery/engagement/system) · schema_version · source · actor_type·actor_id · object_type·object_id · subject_identity_id? · identity_refs(JSONB) · context(JSONB, PII-safe) · occurred_at · ingested_at · contact_id? · execution_id?(causal) · campaign_id?(contextual, nullable, never fabricated) · channel_connection_id? · value_micros?/currency? · idempotency_key(UNIQUE) · provider_event_id? · provider_message_id? · correlation_id · causation_event_id?`.
  `UNIQUE(idempotency_key)` prevents double-counting on webhook retries (identity ≠ dedup key ≠ provider id — three fields). `correlation_id`/`causation_event_id` reconstruct the causal chain (decision → execution → published → click → lead → viewing → transaction). See `docs/event-taxonomy-catalog.md` for the vocabulary + rules.
- **MetricObservation** 🆕 [L schema / FF ingest] — **snapshots are NOT events.** Cumulative platform counters (impressions/reach/likes/shares/views) live here, not in `events`: `id · subject_type · subject_id · execution_id? · metric_name · metric_value · aggregation_type(cumulative/gauge) · period_start · period_end · observed_at · source · provider_ref · dimensions(JSONB)`. Prevents summing cumulative snapshots as if they were discrete events.
