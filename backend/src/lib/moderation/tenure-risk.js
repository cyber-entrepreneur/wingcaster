 ...
 * moderation queue). PA-MOD-002 renders "risk unknown" copy from this
 * payload via `tenureRiskLabel`.
 *
...
/**
 * Score tenure risk for a moderation submission.
 *
 * v1 always returns `{ tier: 'unknown', score: null, signals: [], version: 'v1-stub' }`
 * regardless of input (null agent, missing agency, richly populated records,
 * positional extras, etc.). Never throws on missing data. Never queries the
 * database.
 *
 * Phase 2 will consider agency onboarding age, prior-rejection ratio across
 * portals, portal-fee-history, and WingCaster tenure. v1 must not invent scores.
 *
 * @param {{ agent?: object, submission?: object, agency?: object }} [_input]
 * @returns {{ tier: 'low'|'medium'|'high'|'unknown', score: number|null, signals: string[], version: 'v1-stub' }}
 */
export function scoreTenureRisk(_input) {
 ...
 * always false unless a caller passes a non-stub override — the gate is
 * kept implemented for Phase 2.
 *
 * @param {{ tier?: string } | null | undefined} tenureRisk
 ...
 *
 * Kickoff non-negotiable 3: `unknown` → `'Risk unknown'` so PA-MOD can
 * render that copy without inventing it client-side.
 *
